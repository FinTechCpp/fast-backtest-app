...
#include "generic_strategy.hpp"  // Include the generic strategy class
#include <memory>
#include <vector>
#include <string>
#include <map>
 ...
 */
class GenericStrategyAdapter : public be::Strategy {
private:
    // Specific configuration for the Generic strategy
    GenericStrategyConfig strategy_config;

    // Instance of the Generic strategy
    std::unique_ptr<GenericStrategy> strategy;
...
public:
    ...
    GenericStrategyAdapter(
        ...
        const GenericStrategyConfig& generic_config
    ) : be::Strategy(broker, data), strategy_config(generic_config) {
        // Create Generic Strategy instance with the provided configurations
        strategy = std::make_unique<GenericStrategy>(base_config, generic_config);
        ...
        async_file->info("GenericStrategyAdapter initialized");
        async_file->info("Strategy name: {}", strategy_config.name);
        async_file->info("Direction: {}", strategy_config.go_direction.has_value());
