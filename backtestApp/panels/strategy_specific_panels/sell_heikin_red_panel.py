...
class SellHeikinRedPanel(BasePanel):
    """Panel spécifique à la stratégie SellHeikinRed."""
    
    def __init__(self, parent=None):
        super().__init__(parent)
    
    def create(self):
        """Crée le panel des paramètres spécifiques à SellHeikinRed."""
        strategy_group = QGroupBox("Paramètres SellHeikinRed")
        ...
        self.widgets['stoch_threshold_spin'].setValue(80)  # Seuil plus élevé pour vendre
        ...
        # Section pour le filtre de bougie Heikin Ashi précédente verte
        ha_group = QGroupBox("Condition bougie Heikin Ashi")
        ha_layout = QVBoxLayout()
        
        self.widgets['previous_ha_candle_green_check'] = QCheckBox("Activer le filtre bougie précédente verte")
        self.widgets['previous_ha_candle_green_check'].setChecked(True)
        ha_layout.addWidget(self.widgets['previous_ha_candle_green_check'])
    ...
    def get_values(self):
        ...
        return {
            ...
            'use_previous_ha_candle_green_filter': self.widgets['previous_ha_candle_green_check'].isChecked(),
        }
    
    def get_required_indicators(self):
        """
        Retourne les indicateurs nécessaires pour la stratégie SellHeikinRed.
