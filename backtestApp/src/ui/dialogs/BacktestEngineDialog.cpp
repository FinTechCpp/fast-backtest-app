...
// Implementation of CandleWidget
...
void CandleWidget::paintEvent(QPaintEvent* event)
{
    ...
    int midY = h / 2;
    int labelHeight = 30;
    int offset = 10; // Margin from top and bottom of each candle


    // Position of the first candle (previous)
    int candle1X = (w - m_candleSpacing) / 2;
    
    // Position of the second candle (current)
    int candle2X = (w + m_candleSpacing) / 2;
    
    // Height of the candles
    int candleHeight = h / 2 - 2 * offset;
    
    // Legend above the candles
    ...
    int levelY = midY; // Y position of the order level
    ...
    // Determine colors and positions according to order type
    if (m_type == OrderType::STOP_BUY || m_type == OrderType::LIMIT_SELL) {
        // Bullish green candles
        prevColor = m_bullishColor; // Previous candle green
        curColor = m_bullishColor; // Current candle green
        ...
        // Bearish red candles
        prevColor = m_bearishColor; // Previous candle red
        curColor = m_bearishColor; // Current candle red
        ...
        // The closing level should be at midY
    }
    ...
    // Draw the first candle (previous)
    ...
    // Draw the second candle (current)
    ...
    if (m_type != OrderType::MARKET) {
        ...
        if (m_executeOnLevel) {
            execX -= m_candleWidth; // If execution on level, arrow is on the left
        }
    }


    execX -= 5; // Offset to the left for the arrow
...
BacktestEngineDialog::BacktestEngineDialog(QWidget* parent)
    : QDialog(parent)
{
    setWindowTitle("Advanced backtest configuration");
    setMinimumSize(950, 750);
    
    QVBoxLayout* mainLayout = new QVBoxLayout(this);
    
    // Grid for the top part (2x2)
    QGridLayout* topGridLayout = new QGridLayout();
    topGridLayout->setColumnStretch(0, 1); // Left column takes 50%
    topGridLayout->setColumnStretch(1, 1); // Right column takes 50%
    topGridLayout->setHorizontalSpacing(10);
    topGridLayout->setVerticalSpacing(10);

    // Additional backtest parameters section (left column)
    QGroupBox* additionalParamsGroupBox = new QGroupBox("Additional Parameters", this);
    ...
    // Ratio of spread used for entry price
    ...
    additionalParamsLayout->addRow(new QLabel("Spread ratio for entry price (0.0 - 1.0):", this), m_spreadEntryRatioSpin);
    ...
    additionalParamsLayout->addRow(new QLabel("Initial cash:", this), m_cashSpin);
    ...
    additionalParamsLayout->addRow(new QLabel("Maximum leverage:", this), m_leverageSpin);
    
    // Position Mode
    m_positionModeCombo = new QComboBox(this);
    m_positionModeCombo->addItems({"Hedging", "Netting"});
    additionalParamsLayout->addRow(new QLabel("Position mode:", this), m_positionModeCombo);

    // Minimum position step (quantization)
    ...
    additionalParamsLayout->addRow(new QLabel("Position quantization:", this), m_minPositionStepSpin);
    ...
    // Trade on Close section (right column)
    ...
    QLabel* tradeOnCloseLabel = new QLabel(
        "If enabled, MARKET orders are executed at the previous candle's close price. "
        "Otherwise, they are executed at the current candle's open price.", this);
    tradeOnCloseLabel->setWordWrap(true); // Important: allow automatic line wrap
    tradeOnCloseLayout->addWidget(tradeOnCloseLabel);
    
    QButtonGroup* tradeOnCloseGroup = new QButtonGroup(this);
    m_tradeOnCloseRadio = new QRadioButton("Enable Trade on Close", this);
    m_tradeOnOpenRadio = new QRadioButton("Execute at open price", this);
    ...
    // Grid for the lower part (STOP and LIMIT side by side)
    QGridLayout* bottomGridLayout = new QGridLayout();
    bottomGridLayout->setColumnStretch(0, 1); // Left column takes 50%
    bottomGridLayout->setColumnStretch(1, 1); // Right column takes 50%
    bottomGridLayout->setHorizontalSpacing(10);
    
    // STOP Orders section (left column)
    QGroupBox* stopGroupBox = new QGroupBox("STOP order execution during a gap", this);
    QVBoxLayout* stopLayout = new QVBoxLayout(stopGroupBox);
    
    QLabel* stopLabel = new QLabel("Execution price level:", this);
    stopLayout->addWidget(stopLabel);
    
    QButtonGroup* stopGroup = new QButtonGroup(this);
    m_stopOnOpenRadio = new QRadioButton("Open price (worst case)", this);
    m_stopOnLevelRadio = new QRadioButton("STOP level", this);
    ...
    // LIMIT Orders section (right column)
    QGroupBox* limitGroupBox = new QGroupBox("LIMIT order execution during a gap", this);
    QVBoxLayout* limitLayout = new QVBoxLayout(limitGroupBox);
    
    QLabel* limitLabel = new QLabel("Execution price level:", this);
    limitLayout->addWidget(limitLabel);
    
    QButtonGroup* limitGroup = new QButtonGroup(this);
    m_limitOnLevelRadio = new QRadioButton("LIMIT level (worst case)", this);
    m_limitOnOpenRadio = new QRadioButton("Open price", this);
