...
void FilterEditDialog::setupOperatorUI(QWidget* parent)
{
    ...
    // Distance (threshold) between left and right values
    ...
    m_distanceSpin->setToolTip("Distance minimale entre les deux valeurs pour que la condition soit vraie (0 = pas de contrainte)");
    
    // Créer un label pour la distance
    m_distanceLabel = new QLabel("Distance (seuil):", parent);
...
void FilterEditDialog::setFilter(const filter::GenericFilter& filter)
{
    ...
    // Charger la distance
    m_distanceSpin->setValue(filter.distance);
...
void FilterEditDialog::onOkButtonClicked()
{
    ...
    m_filter.distance = m_distanceSpin->value();
