...
void EquityCurveWidget::updateData(const be::Stats& stats) {
    try {
        ...
        // Ajouter la série au graphique (c'est ici que le segment fault se produisait)
        if (equitySeries && !equitySeries->points().empty()) {
            chart->addSeries(equitySeries);
        } else {
            qWarning() << "Série vide ou invalide, impossible de l'ajouter au graphique";
            delete equitySeries;
            delete chart;
            return;
        }
        ...
        qCritical() << "Exception dans EquityCurveWidget::updateData:" << e.what();
    } catch (...) {
        qCritical() << "Exception inconnue dans EquityCurveWidget::updateData";
    }
}
