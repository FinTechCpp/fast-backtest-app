...
void ResultManager::updateAllViews(void* data, void* stats)
{
    qDebug() << "updateAll appelé avec des pointeurs opaques data et stats";
    
    qDebug() << "updateAllViews appelé avec des pointeurs data et stats";
    
    try {
        for (auto it = m_views.begin(); it != m_views.end(); ++it) {
            BaseView* view = it.value();
            if (view) {
                view->updateData(data, stats);  // Changé de update() à updateData()
            }
        }
    }
