...
# Vérifier Qt 6
if ! command -v qmake6 &> /dev/null && ! command -v qmake-qt${QT_VERSION} &> /dev/null && ! command -v qmake &> /dev/null; then
    echo "Erreur: qmake pour Qt6 non trouvé. Installez Qt6."
    echo "Essayez: sudo apt install qt6-base-dev qt6-tools-dev"
    exit 1
fi
...
# Vérifier qchartviewer.h dans le bon répertoire
...
# Générer le Makefile avec qmake Qt6
echo "Génération du Makefile avec Qt6..."
if command -v qmake6 &> /dev/null; then
    qmake6 "$PROJECT_DIR/backtest_app_cpp.pro"
elif command -v qmake-qt${QT_VERSION} &> /dev/null; then
    qmake-qt${QT_VERSION} "$PROJECT_DIR/backtest_app_cpp.pro"
elif command -v qmake &> /dev/null; then
    # Vérifier que c'est bien Qt6
    if qmake -v | grep -q "Qt version 6"; then
        qmake "$PROJECT_DIR/backtest_app_cpp.pro"
    else
        echo "Erreur: qmake pointe vers Qt5, pas Qt6"
        exit 1
    fi
else
    echo "Erreur: Aucune version de qmake Qt6 trouvée"
    exit 1
fi
