...
def search_market(ig_service):
    ...
    search_term = input(f"Entrez le marché à rechercher : {search_term_default}") or f"{search_term_default}"
...
def get_market_info(ig_service):
    ...
    epic = input(f"Entrez l'EPIC du marché : {epic_default}") or f"{epic_default}"
...
def track_realtime_prices(ig_service):
    ...
    epic = input("Entrez l'EPIC du marché (ex: IX.D.NASDAQ.IFE.IP): ")
...
def create_position(ig_service):
    ...
    epic = input("Entrez l'EPIC du marché: ")
